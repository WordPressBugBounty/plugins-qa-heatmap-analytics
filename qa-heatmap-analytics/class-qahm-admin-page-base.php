...
class QAHM_Admin_Page_Base extends QAHM_File_Data {
	...
	protected function common_enqueue_script() {
		...
		if( ! $this->wrap_get_option( 'plugin_first_launch' ) && ! $this->is_maintenance() ) {
			wp_enqueue_script( QAHM_NAME . '-admin-page-base',  $js_dir_url . 'admin-page-base.js', array( QAHM_NAME . '-effect' ), QAHM_PLUGIN_VERSION );
		}
	}
	...
	protected function view_not_enqueue_jquery_html() {
		$msg = printf( esc_html__( 'Since jquery is not loaded, the function of'. QAHM_PLUGIN_NAME.' cannot be enabled. %s Use "wp_enqueue_script" to load jquery.', 'qa-heatmap-analytics' ), '<br>' );
		echo <<< EOH
			<div id="qahm-error" class="error notice is-dismissible">
				<p>{$msg}</p>
			</div>
EOH;
	}
...
EOD;
		...
		$mes .= '<p>' . sprintf( esc_html__( 'If you continue to see this notice for an extended period, please refer to our %1$stroubleshooting page%2$s.', 'qa-heatmap-analytics' ), '<a href="https://mem.quarka.org/en/manual/keep-getting-data-is-under-maintenance/" target="_blank" rel="noopener">', '</a>' ) . '</p>';
		$mes .= '</div>';
		echo $style;
		echo $mes;
	}
	...
	protected function view_rss_feed() {
		...
		if ( $wp_lang_set === 'ja' ) {
			...
			if ( ! empty( $maxitems ) && $maxitems > 0 ) {
				...
				<?php //キャンペーン告知窓読込み
				include_once plugin_dir_path( __FILE__ ) . 'qahm-adv.php';
				?>

					<div class="postbox">
						<h2><?php esc_html_e( 'QA Analytics News and Announcements', 'qa-heatmap-analytics' ); ?></h2>
						<div class="rss-widget">
							<h3><?php echo esc_html__( 'Installed Version', 'qa-heatmap-analytics' ) . ': ' . QAHM_PLUGIN_VERSION; ?></h3>
							<ul>
